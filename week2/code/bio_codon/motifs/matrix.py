...
class PositionSpecificScoringMatrix(GenericPositionMatrix):
    ...
    def calculate(self, sequence):
        ...
        scores = np.zeros(n - m + 1, dtype=np.float32)
        
        # Iterate through the sequence (Pure Python/Codon implementation)
        for i in range(n - m + 1):
            score = 0.0
            for j in range(m):
                letter = seq_str[i + j]
                # In Codon/Python, accessing the dict is standard. 
                # If letter is 'N' or unknown, this might crash or return 0 
                # depending on your matrix. Assuming standard ACGT:
